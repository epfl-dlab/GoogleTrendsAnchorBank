...
######################## TODO: Use FB as ref query again!
######################## TODO: Change choice of first anchor in binsearch!
...
```{r}
plot(log(sort(ab$W0[ref_anchor,])), panel.first=grid())
bottom_anchor <- names(which.min(ab$W0[ref_anchor,]))
es <- ab$W0_paths[[ref_anchor]][[which(names(ab$W0_paths)==bottom_anchor)]]
...
abline(h=log(ab$W[ref_anchor,]), col='red')
# Bold red: reference anchor.
abline(h=log(ab$W[ref_anchor,ref_anchor]), col='red', lwd=3)
```
...
ts_repr <- ts_repr[,names(anchor_bank)]
ts_repr <- apply(ts_repr, 2, function(c) c/max(c))
ts_repr <- t(t(ts_repr) * anchor_bank)
matplot(ts_repr, type='l', lty=1, log='y', col=1:6)
legend('topright', mid2name(names(anchor_bank)), lty=1, col=1:6, bty='n')

plot_anchor_bank <- function(anchor_bank, xoff_refquery, xoff_navquery) {
  ...
  text(1 + xoff_refquery, 1, mid2name(names(y[1])), adj=c(0,0), col=col_ref, srt=30)

  # Nav queries
  points(navquery_idx, y[navquery_idx], lwd=1, col=col_nav)
  text(navquery_idx + xoff_navquery, y[navquery_idx], mid2name(names(y[navquery_idx])), adj=c(-0.1,0), srt=30, col=col_nav)
}

# Plot ratios w.r.t. top keyword.
if (SAVE_PLOTS) pdf(sprintf('%s/anchor_ratios.pdf', PLOT_DIR), width=2, height=3.25, pointsize=6,
                      family='Helvetica', useDingbats=FALSE)
plot_anchor_bank(anchor_bank_init, xoff_refquery=4, xoff_navquery=c(0,5,0,0,5))
idx <- seq(17, 57, 10)
text(idx, anchor_bank_init[idx], mid2name(names(anchor_bank_init[idx])), adj=c(-0.1,0), srt=30, col=col_rest)
idx <- seq(67, length(anchor_bank_init), 10)
text(idx, anchor_bank_init[idx], mid2name(names(anchor_bank_init[idx])), adj=c(1.2,1), srt=30, col=col_rest)
if (SAVE_PLOTS) dev.off()

########################################
plot_anchor_bank(anchor_bank, xoff_refquery=0.1, xoff_navquery=c(0,0))
idx <- 4:11
text(idx, anchor_bank[idx], mid2name(names(anchor_bank[idx])), adj=c(-0.1,0), srt=30, col=col_rest)
########################################
