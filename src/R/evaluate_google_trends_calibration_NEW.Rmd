...
f <- function(r, R) {
  logR <- log(R)/log(r)
  res <- R / r^(ceiling(logR)-1)
  ((1+eps*r)/(1-eps*r))^(ceiling(logR)-1) * (1+eps*res)/(1-eps*res)
}
F <- function(r, R) ((1+eps*r)/(1-eps*r))^(log(R)/log(r))

# Plot f(r,R) as a function of r, for various choices of R.
Rs <-10^seq(0, 7, 0.01)
rs <-seq(1.4, 10, 0.01)
e <- exp(1)
par(mfrow=c(2,3))
for (R in 10^(1:6)) {
  ylim=c(1, max(F(rs,10^6)))
  plot(rs, f(rs,R), log='', type='l', main=sprintf('R = 1e%d', log10(R)), ylim=ylim,
       panel.first=c(grid(), lines(rs, F(rs,R), col='red'), abline(v=e, col='green')))
}
# Plot f as a function of R, for the optimal choice r=e.
par(mfrow=c(1,1))
plot(Rs, f(e,Rs), log='x', type='l', panel.first=c(grid(), lines(Rs, F(e,Rs), col='red')))
```
