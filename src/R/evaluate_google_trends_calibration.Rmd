...
ts <- cbind(result1$ts, result2$ts[,extra_idx])
iter <- c(result1$iter, result2$iter[extra_idx])
ratio <- c(result1$ratio, result2$ratio[extra_idx])
ts <- cbind(ts, result_arnstorf$ts)
colnames(ts)[ncol(ts)] <- mid_arnstorf
ts[,mid_arnstorf] <- result_arnstorf$ts
iter[mid_arnstorf] <- result_arnstorf$iter
ratio[mid_arnstorf] <- result_arnstorf$ratio

matplot(result_highlights$ts, type='l', lty=1, log='', col=colorblind_col, lwd=3, bty='n',
        xlab='Week of 2019', ylab='Normalized search volume')
legend('topright', names(highlights), lty=1, col=colorblind_col, lwd=3, bty='n', inset=c(0,0.4))

matplot(1+result_highlights$ts, type='l', lty=1, log='y', col=colorblind_col, lwd=3, bty='n',
        xlab='Week of 2019', ylab='Normalized search volume')
legend('topright', names(highlights), lty=1, col=colorblind_col, lwd=3, bty='n', inset=c(0,0.4))

idx <- order(ratio, decreasing=TRUE)
...
for (h in highlights) {
  ...
  lines(ts[,h], lwd=3, col=col)
  i <- i + 1
}
legend('topright', names(highlights), lty=1, col=colorblind_col, lwd=3, bty='n', inset=c(0,0.1))
...
barplot(tapply(iter, cut(ratio, c(0, quantile(ratio))), mean))
...
matplot(ts[,idx], type='l', lty=1, log='y', col=rgb(0,0,0,0.1), lwd=2, bty='n',
        xlab='Week of 2019', ylab='Calibrated search volume')
lines(apply(ts, 1, median), type='l', lwd=4, col=colorblind_col[1])
lines(bs[1,], type='l', lwd=2)
lines(bs[3,], type='l', lwd=2)
...
iter_all <- c(iter_bavaria, iter_soccer)
