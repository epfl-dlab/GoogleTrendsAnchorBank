...
This is the script to produce all results of the following paper:
...
If you are interested in using Google Trends Anchor Bank (G-TAB) in your own research, please use the Python implementation available at `[https://github.com/epfl-dlab/GoogleTrendsAnchorBank](https://github.com/epfl-dlab/GoogleTrendsAnchorBank)`.
...
```{r}
if (SAVE_PLOTS) pdf(sprintf('%s/anchor_bank.pdf', PLOT_DIR), width=3.4, height=3.25, pointsize=6,
...
```{r}
if (SAVE_PLOTS) pdf(sprintf('%s/eta_as_fct_of_c.pdf', PLOT_DIR), width=3.4, height=3.25, pointsize=6,
                      family='Helvetica', useDingbats=FALSE)
c <- seq(0.1, 0.72, 0.01)
par(mfrow=c(2,3))
for (r in 0.1^(2:7)) {
...
if (SAVE_PLOTS) pdf(sprintf('%s/eta_bar.pdf', PLOT_DIR), width=3.4, height=3.25, pointsize=6,
                      family='Helvetica', useDingbats=FALSE)
par(mfrow=c(1,1))
